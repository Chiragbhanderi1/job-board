...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="mx-auto mt-10 max-w-2xl bg-gradient-to-r from-indigo-200 from-10% via-sky-200 via-30% to-emerald-200 to-90%  text-slate-700">
        ...
        @if (session('success'))
            <div class="my-8 rounded-sm border-l-4 border-green-400 bg-green-100 p-4 text-green-600 opacity-75">
                <p class="font-bold">Success!</p>
                <p>{{session('success')}}</p>
            </div>
        @endif
        @if (session('error'))
            <div class="my-8 rounded-sm border-l-4 border-red-400 bg-red-100 p-4 text-red-600 opacity-75">
