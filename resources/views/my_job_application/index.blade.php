    ...
    <x-job-card :job="$application->job">
        <div class="flex items-center justify-between text-sm text-slate-500">
            <div class="">
                ...
                <div> {{$application->job->job_applications_count - 1}} other {{Str::plural('applicant',$application->job->job_applications_count -1)}}</div>
                <div>Your asking salary is ${{number_format($application->expected_salary)}}</div>
                <div>Average asking salary is ${{number_format($application->job->job_applications_avg_expected_salary)}}</div>
            </div>
            <div class="">
